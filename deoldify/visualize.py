...
#ani version
def get_image_colorizer_ani(
    render_factor: int = 35, artistic: bool = True
) -> ModelImageVisualizer:
    if artistic:
        return get_artistic_image_colorizer_ani(render_factor=render_factor)
    else:
        return get_stable_image_colorizer(render_factor=render_factor)
...
def get_artistic_image_colorizer(
    ...
    return vis
#ani version
def get_artistic_image_colorizer_ani(
    root_folder: Path = Path('./'),
    weights_name: str = 'ColorizeArtistic_gen_ani',
    results_dir='result_images',
    render_factor: int = 35,
) -> ModelImageVisualizer:
    learn = gen_inference_deep(root_folder=root_folder, weights_name=weights_name)
